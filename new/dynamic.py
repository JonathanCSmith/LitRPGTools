...
class DynamicDataStore:
    def __init__(self, engine: 'LitRPGToolsEngine'):
        ...
        self.__value_store: Dict[str, Dict[int, Dict[str, str]]] = dict()

    def translate(self, entry_id: str, input_string: str) -> str:
        keys = self.__find_special_variables(input_string)
        return self.__translate_with_keys_for_entry(entry_id, input_string, keys)
    ...
    def update(self):
        ...
        initialisation_value_store = dict()
        for character in characters:
            initialisation_value_store[character.unique_id] = dict()

            # Add our 'global' category data
            for category_id in character.categories:
                category = self.__engine.get_category_by_id(category_id)
                initialisation_value_store[character.unique_id].update(category.dynamic_data_initialisations)
        ...
        for index, entry_id in enumerate(history_ids):
            ...
            if index == 0:
                for character in characters:
                    ...
                    self.__value_store[character.unique_id][index].update(initialisation_value_store[character.unique_id])
                    rolling_window[character.unique_id] = list()

            # Entries are character specific
            character_id = entry.category_id

            # Note, because entries are character specific we actually get some optimisations here for free in that we don't
            # need to recompute the dynamic cache per character per entry
            ...
            # Now we loop through our rolling window's contents
            to_remove = list()
            for rolling_window_entry_id in rolling_window[character_id]:
                ...
                # Build a list of our modifications and enact based on dependencies
                for dynamic_data_key, (operation_string, operation_type) in entry.dynamic_data_operations:
                    ...
                    modification = Modification(dynamic_data_key, operation_type, operation_string, dependencies)
                    modification_queue.put(modification)
            ...
            # Calculate the effective outcome of our caches
            modifications_since_last_processed = 0
            while modification_queue.not_empty and modifications_since_last_processed != modification_queue.qsize():
                modification = modification_queue.get()
                ...
                # Reset our counter
                modifications_since_last_processed = 0
                self.__perform_modification_for_entry(entry, modification)
    ...
    def __perform_modification_for_entry_at_index(self, index: int, entry: Entry, modification: Modification):
        try:
            # Check if our target key exists
            if modification.key not in self.__value_store[entry.character_id][index]:
                raise KeyError(modification.key + " not in dynamic data store when attempting to target for modification for character: " + entry.character_id + " at index: " + str(index))

            # All of our dependencies are theoretically resolved, so now we should evaluate our expression
            operation_string = self.__translate_with_keys_for_entry(entry.unique_id, modification.operation, modification.dependencies)
            outcome = eval(operation_string, {'__builtins__': None})

            # Match by operation type
            match modification.operation_type:
                case "ASSIGN":
                    self.__value_store[entry.character_id][index][modification.key] = outcome

                case "ADD INTEGER":
                    self.__value_store[entry.character_id][index][modification.key] += int(outcome)

                case "ADD FLOAT":
                    self.__value_store[entry.character_id][index][modification.key] += float(outcome)
            ...
            print("Unable to perform modification of key: " + modification.key + "for entry: " + entry.unique_id + "for character: " + entry.character_id + " at index: " + str(index) + " with source modification: " + modification.operation + ". Reported exception was: " + str(ex))
