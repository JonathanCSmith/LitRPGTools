    ...
    def resizeEvent(self, event: QResizeEvent) -> None:
        ...
        for row_index in range(header_rows + category_rows + footer_rows):
            ...
            if row_index < header_rows - 1:
                ...
                self.__data_movement_layout.itemAt(0, QFormLayout.ItemRole.FieldRole).widget().setFixedHeight(header_height)
                ...
                self.__data_movement_layout.itemAt(row_index - header_rows + 1, QFormLayout.ItemRole.FieldRole).widget().setFixedHeight(desired_height)
...
def create_entry_form(root_gui: 'DesktopGUI', target_layout: QFormLayout, character: Character | None, category: Category, entry: Entry | None, history_index: int, header=True, readonly=False, translate_with_dynamic_data=False, dynamic_data_index=None) -> list:
    ...
    if header:
        ...
        target_layout.addRow("Unique Id:", QLabel(entry.unique_id))
    ...
    if entry is not None and entry.dynamic_data_operations:
        ...
        dynamic_data_components.fill_dynamic_modifications_table(series_data_table, {}, readonly=readonly)
    target_layout.addRow("Dynamic Data:", series_data_table)
    callback_for_editables.append(series_data_table)
