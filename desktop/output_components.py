...
class OutputsTab(VisibleDynamicSplitPanel):
    def __init__(self, root_gui_object: 'LitRPGToolsDesktopGUI'):
        ...
        # Main components
        self._sidebar_widget = OutputSidebar(self.root_gui_object, self)
        self._view_widget = OutputView(self.root_gui_object, self)
...
class OutputSidebar(QWidget):
    def __init__(self, root_gui_object: 'LitRPGToolsDesktopGUI', parent_gui_object: OutputsTab):
        super().__init__()
        self.root_gui_object = root_gui_object
    ...
    def __fill_output_selector(self):
        ...
        outputs = self.root_gui_object.data_manager.get_outputs()
    ...
    def __get_list_row_colour_from_context(self, entry_index) -> Qt.GlobalColor:
        ...
        if self.root_gui_object.data_manager.get_current_history_index() == entry_index:
            return Qt.GlobalColor.blue

        output = self.parent_gui_object.output
    ...
    def draw(self):
        ...
        if output_id is None:
            return
        output = self.root_gui_object.data_manager.get_output_by_id(output_id)
        if output is None:
            return

        # Set our result
        self.parent_gui_object.output = output
...
class OutputView(QScrollArea):
    ...
    def __draw_output(self, output: Output):
        ...
        currently_selected = self.parent_gui_object.get_current_selection()
    ...
    def __handle_move_output_end_up_callback(self, output: Output):
        ...
        self.root_gui_object.data_manager.move_output_target_up_by_id(output.unique_id)
        self.parent_gui_object.draw()
    ...
    def __handle_move_output_end_down_callback(self, output: Output):
        ...
        self.root_gui_object.data_manager.move_output_target_down_by_id(output.unique_id)
        self.parent_gui_object.draw()

    def __handle_edit_output_callback(self, output: Output):
        add_or_edit_output(self.root_gui_object, output)
        self.parent_gui_object.draw()

    def __handle_delete_output_callback(self, output: Output):
        delete_output(self.root_gui_object, output)
        self.parent_gui_object.draw()
    ...
    def __handle_move_entry_up_callback(self, entry_id: str, state: bool):
        ...
        self.root_gui_object.data_manager.edit_output(output)
        self.parent_gui_object.draw()
        self.parent_gui_object.set_curently_selected(current_index - 1)
...
class OutputDialog(QDialog):
    def __init__(self, gui: 'LitRPGToolsDesktopGUI', output: Output):
        ...
        self.__target_gsheet.addItems(self.gui.data_manager.get_unassigned_gsheets())
    ...
    def __handle_done_callback(self):
        outputs = self.gui.data_manager.get_outputs()
...
class OutputSelectorDialog(QDialog):
    def __init__(self, gui: 'LitRPGToolsDesktopGUI'):
        ...
        for index, view in enumerate(self.parent_gui.data_manager.get_outputs()):
...
def add_or_edit_output(gui: 'LitRPGToolsDesktopGUI', original_output: Output | None):
    # Validate gsheets environment
    if gui.data_manager.get_unassigned_gsheets() is None:
    ...
    if original_output is None:
        entry_id = gui.data_manager.get_most_recent_entry_id()
    ...
    if original_output is None:
        gui.data_manager.add_output_to_head(output_dialog.output)
...
def delete_output(gui: 'LitRPGToolsDesktopGUI', output: Output):
    ...
    gui.data_manager.delete_output(output)
